...
remDr$navigate("https://pathe.ch/en/cinema-basel/movies-upcoming.html#/")
...
names <- read_html(remDr$getPageSource()[[1]]) %>%
    html_nodes(".m-teaser-upcoming__title") %>%
    html_text()

dates <- read_html(remDr$getPageSource()[[1]]) %>%
    html_nodes(".a-release-date") %>%
    html_text()
